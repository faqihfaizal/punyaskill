...
export default function EditProfessor() {
        ...
        const fetchData = async () => {
            try {
                ...
                setForm({
                    ...
                    foto_instruktur: res.data.foto_instruktur, // reset, karena backend biasanya kirim path bukan file
                });
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.foto_instruktur && typeof form.foto_instruktur !== "string") {
            data.append("foto_instruktur", form.foto_instruktur);
        }
        try {
            const res = await api.put(`/api/instruktur/${id_instruktur}`, data, {
                headers: {
    ...
    const handleDelete = async () => {
        if (window.confirm(`Yakin ingin menghapus ${form.nama_instruktur}?`)) {
            try {
                await api.delete(`api/instruktur/${id_instruktur}`);
            ...
            <Row>
                <Col xs={12} md={12}>
                    ...
                    <section className="box">
                        ...
                        {form.foto_instruktur && typeof form.foto_instruktur === "string" && (
                            <div className="mt-2">
                                <Label>Foto Saat Ini:</Label><br />
                                <img
                                    src={`http://localhost:5000${form.foto_instruktur}`}
                                    alt="foto instruktur"
                                    width="150"
